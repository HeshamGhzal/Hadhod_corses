...
string string_date_format(sDate date, string format);
...
int main()
{
	...
	cout << d;*/
	cout << "string date : " <<
		string_date_format(date1, format) << endl;
...
vector<string> spelt_each_word_in_string(string s, string separator)
{
	vector<string> vs;
...
sDate string_date_to_struct(string date)
{
	sDate Date;
	vector<string> vdate = spelt_each_word_in_string(date, "/");
...
string date_struct_to_string(sDate date)
{
	return to_string(date.day) + "/" + to_string(date.month) + "/" + to_string(date.year);
}

string replace_word_in_string(string s, string to_replac, string replace_to)
{
	short pos = s.find(to_replac);
	while (pos != string::npos)
	{
		s.replace(pos, to_replac.length(), replace_to);
		pos = s.find(to_replac);
	}
	return s;
}
...
string string_date_format(sDate date, string format)
{
	format = replace_word_in_string(format, "dd", to_string(date.day));
	format = replace_word_in_string(format, "mm", to_string(date.month));
	format = replace_word_in_string(format, "yyyy", to_string(date.year));
	return format;
}